          ...
          {Array.from(byProperty.entries()).map(([propId, group]) => (
            <div key={propId} className={v2 ? "overflow-hidden border border-[#d4dde7] bg-white" : "bg-card border border-border rounded-lg overflow-hidden"}>
              ...
              {v2 ? <div className="divide-y divide-[#e1e6ec]">
                {group.users!.map(entry => <div key={entry.userId} className="flex items-center gap-2 px-3 py-2.5 sm:gap-3 sm:px-5 sm:py-3">
                  <div className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
                    <span className="grid size-8 shrink-0 place-items-center bg-[#edf1f5] text-[13px] font-bold text-[#173f70] sm:size-10 sm:text-[14px]">{(entry.userName || "?").charAt(0)}</span>
                    <div className="min-w-0"><div className="flex flex-wrap items-center gap-x-2 gap-y-1"><p className="text-[14px] font-bold text-[#102d50] sm:text-[15px]">{entry.userName || "—"}</p>{entry.showCompany !== 0 && entry.userCompany && <span className="text-[12px] text-[#65748a] sm:text-[13px]">{entry.userCompany}</span>}{entry.verified === 1 && <span className="hidden items-center gap-1 bg-[#e9f1f8] px-2 py-0.5 text-[10px] font-bold text-[#173f70] sm:flex"><CheckCircle2 className="size-3"/>認証済み</span>}</div><div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1">{entry.types.includes("favorite") && <span className="flex items-center gap-1 text-[11px] text-[#a13b50]"><Heart className="size-3 fill-current"/>お気に入り</span>}{entry.types.includes("memo") && <span className="flex items-center gap-1 text-[11px] text-[#8b5a08]"><StickyNote className="size-3"/>メモあり</span>}{entry.types.includes("document") && <span className="flex items-center gap-1 text-[11px] text-[#17685a]"><FileText className="size-3"/>PDF資料作成{entry.documentCount > 1 ? ` ${entry.documentCount}回` : ""}</span>}{entry.viewCount > 0 && <span className="flex items-center gap-1 text-[11px] text-[#65748a]"><Eye className="size-3"/>閲覧{entry.viewCount}回</span>}{entry.types.includes("dm") && <span className={`px-1.5 py-0.5 text-[10px] font-bold sm:hidden ${entry.propertyStatus === "sold" ? "bg-[#eceff2] text-[#526176]" : "bg-[#fff1b8] text-[#765500]"}`}>{entry.propertyStatus === "sold" ? "成約済み" : "問合せあり"}</span>}</div></div>
                  </div>
                  <div className="flex shrink-0 items-center gap-2"><span className="hidden sm:inline">{entry.types.includes("dm") && (entry.propertyStatus === "sold" ? <span className="bg-[#eceff2] px-2 py-1 text-[11px] font-bold text-[#526176]">成約済み</span> : <span className="bg-[#fff1b8] px-2 py-1 text-[11px] font-bold text-[#765500]">問合せあり</span>)}</span>{entry.types.includes("dm") && <button onClick={() => setLocation(`/v2/chat/${entry.userId}/${entry.propertyId}`)} className="flex h-9 items-center gap-1 border border-[#173f70] px-2 text-[11px] font-bold text-[#173f70] sm:h-10 sm:gap-1.5 sm:px-3 sm:text-[12px]"><MessageCircle className="size-3.5"/><span className="hidden min-[370px]:inline">問い合わせを見る</span><span className="min-[370px]:hidden">DM</span></button>}</div>
                </div>)}
              </div> : <>
                    ...
                    <tr key={entry.userId} className="hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          {entry.types.includes("dm") && <span className={`whitespace-nowrap px-2 py-1 text-xs font-semibold ${entry.propertyStatus === "sold" ? "bg-gray-100 text-gray-600" : "bg-amber-100 text-amber-800"}`}>{entry.propertyStatus === "sold" ? "成約済み" : "問合せあり"}</span>}
