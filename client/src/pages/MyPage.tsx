...
export default function MyPage({ v2 = false }: { v2?: boolean }) {
  ...
  const { data: deletedProperties, isLoading: deletedLoading } =
    trpc.mypage.deletedProperties.useQuery();
  ...
  return (
    <div className={v2 ? "max-w-none space-y-5" : "space-y-6 max-w-4xl"}>
          ...
          <div className="border-t border-primary/10 p-4 space-y-4">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-white bg-gray-800 rounded px-1.5 py-0.5">
                  iPhone
                </span>
              ...
              <ol className="text-xs text-muted-foreground space-y-1.5 ml-1">
                ...
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    画面下部の <strong>共有ボタン（□に↑）</strong> をタップ
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="bg-primary/10 text-primary rounded-full w-5 h-5 flex items-center justify-center shrink-0 text-[10px] font-bold mt-0.5">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-white bg-[#3DDC84] rounded px-1.5 py-0.5">
                  Android
                </span>
                <span className="text-sm font-medium text-foreground">
                  Androidの場合
              ...
              <ol className="text-xs text-muted-foreground space-y-1.5 ml-1">
                ...
                <li className="flex items-start gap-2">
                  ...
                  <span>
                    画面上部の「<strong>インストール</strong>
                    」バナー、または右上の <strong>︙メニュー</strong> をタップ
                  </span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="bg-primary/10 text-primary rounded-full w-5 h-5 flex items-center justify-center shrink-0 text-[10px] font-bold mt-0.5">
            ...
            <p className="text-[11px] text-muted-foreground/60">
              ※
              インストール後はアプリとして起動でき、ブラウザを閉じていても通知を受け取れます
            </p>
          </div>
        </div>
      )}
      ...
      <div
        className={
          v2
            ? "border border-[#d4dde7] bg-white p-5"
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
          <div className="flex items-center gap-4">
            <div
              className={
                v2
                  ? "flex items-center gap-2 text-[15px] font-bold text-[#102d50]"
          ...
          <div className="flex items-center gap-2">
            <label
              className={
                v2
                  ? "inline-flex h-10 cursor-pointer items-center gap-2 border border-[#173f70] px-4 text-[13px] font-bold text-[#173f70]"
                  : "cursor-pointer inline-flex items-center gap-1.5 text-xs border border-border rounded-md px-3 py-1.5 hover:border-primary hover:text-primary transition-colors"
              }
            >
              <input
                type="file"
            ...
            {user.logoBase64 && (
              <Button
                ...
                onClick={() => logoMutation.mutate({ logoBase64: null })}
        ...
        <p className="text-xs text-muted-foreground mt-2">
          ※ PDF出力時に会社ロゴとして使用されます（推奨: 横長PNG/JPG、2MB以下）
        </p>
      </div>
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex items-center gap-4">
            ...
            {(user as any).businessCardBase64 ? (
              <img
                src={`data:image/jpeg;base64,${(user as any).businessCardBase64}`}
                alt="名刺"
          ...
          <div className="flex items-center gap-2">
            ...
            {(user as any).businessCardBase64 && (
              <Button
                ...
                className="gap-1.5 text-xs text-muted-foreground"
                onClick={() =>
                  cardMutation.mutate({ businessCardBase64: null })
                }
        ...
        {(user as any).verified && (user as any).businessCardBase64 ? (
          ...
          <p className="mt-2 flex items-center gap-1 text-xs font-medium text-[#65748a]">
            <CheckCircle2 className="size-3.5" />
            名刺を登録すると、認証済み業者として表示されます
          </p>
        ) : (user as any).businessCardBase64 ? (
          <p className="text-xs text-muted-foreground mt-2">
            名刺を確認後、管理者が認証マークを付与します
          </p>
        ) : (
          <p className="text-xs text-amber-600 mt-2">
            💡
            名刺を登録し、管理側が確認できますと、物件詳細やメッセージに「認証マーク」が表示されます。さらにDM画面から相手にワンクリックで名刺を送れるようにもなります
          </p>
        )}
      </div>
      ...
      {/* 非表示物件 */}
      <div
        className={v2 ? "overflow-hidden border border-[#d4dde7] bg-white" : ""}
      >
        <div
          className={
            v2 ? "border-b border-[#d4dde7] bg-[#edf1f5] px-5 py-4" : ""
          }
        >
          <h2
            className={
              v2
                ? "flex items-center gap-2 text-[15px] font-bold text-[#102d50]"
                : "mb-4 flex items-center gap-2 text-lg font-semibold text-foreground"
            }
          >
            <EyeOff
              className={
                v2 ? "size-4 text-[#173f70]" : "w-5 h-5 text-muted-foreground"
              }
            />
            削除した物件
          </h2>
        ...
        <div className={v2 ? "p-4 lg:p-5" : ""}>
          {deletedLoading ? (
            ...
            <div
              className={
                v2
                  ? "border border-[#e2e7ec] py-8 text-center"
                  : "bg-card border border-border rounded-lg py-10 text-center"
              }
            >
              ...
              {deletedProperties.map(prop => {
                const remainingDays = prop.ownerDeletedAt
                  ? Math.max(
                      0,
                      Math.ceil(
                        (new Date(prop.ownerDeletedAt).getTime() +
                          30 * 86400000 -
                          Date.now()) /
                          86400000
                      )
                ...
                return (
                  <div
                    ...
                    className={
                      v2
                        ? "border border-[#d9e0e8] p-4"
                        : "bg-card border border-border rounded-lg p-4 opacity-70"
                    }
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-right shrink-0 flex items-center gap-3">
                        ...
                        <Button
                          ...
                          className={
                            v2
                              ? "h-10 gap-1.5 rounded-none border-[#173f70] px-4 text-xs font-bold text-[#173f70]"
                              : "gap-1.5 text-xs"
                          }
      ...
      {restoreTarget && (
        ...
        >
          <div
            className="w-full bg-white p-5 sm:max-w-md sm:border-t-4 sm:border-t-[#173f70]"
            onClick={event => event.stopPropagation()}
          >
            <h3 className="text-[19px] font-bold text-[#102d50]">物件を復元</h3>
            <p className="mt-2 text-[13px] leading-6 text-[#526176]">
              「{restoreTarget.name}」を物件一覧へ戻します。
            </p>
            ...
            <div className="mt-5 flex gap-3">
              ...
              <button
                disabled={
                  ...
                  (notifyRestore && !restoreMessage.trim())
                }
                onClick={async () => {
                  const result = await restoreMutation.mutateAsync({
                    id: restoreTarget.id,
                    notifyPartners: notifyRestore,
                    message: notifyRestore ? restoreMessage.trim() : undefined,
                  });
                  if (!result.success) {
                    alert(
                      result.expired
...
function VisibilitySettings({ v2 = false }: { v2?: boolean }) {
  const { data: settings, isLoading } =
    trpc.auth.getVisibilitySettings.useQuery();
  ...
  const items = [{ key: "showCompany", label: "会社名" }];

  return (
    <div
      className={
        v2
          ? "overflow-hidden border border-[#d4dde7] bg-white"
      ...
      <div
        className={
          v2
            ...
            : "px-5 py-3 border-b border-border bg-muted/40"
        }
      >
        ...
        {items.map(item => (
          ...
          >
            <input
              ...
              checked={(vals as any)[item.key] === 1}
              onChange={e => save(item.key, e.target.checked ? 1 : 0)}
            />
...
function NotifySettings({ v2 = false }: { v2?: boolean }) {
  ...
  const [notifyPropertySearch, setNotifyPropertySearch] = useState(1);
  ...
  useEffect(() => {
    if (settings && !loaded) {
      ...
      setNotifyPropertySearch(settings.notifyPropertySearch);
  ...
  const save = async (np: number, ps: number, dm: number, an: number) => {
    setNotifyNewProperty(np);
    setNotifyPropertySearch(ps);
    setNotifyDm(dm);
    setNotifyAnnounce(an);
    await mutation.mutateAsync({
      notifyNewProperty: np,
      notifyPropertySearch: ps,
      notifyDm: dm,
      notifyAnnounce: an,
    });
      ...
      <div className="p-5 space-y-3">
        <label className="flex items-center gap-3 cursor-pointer">
          <input
            type="checkbox"
            className="accent-primary w-4 h-4"
            checked={notifyNewProperty === 1}
            onChange={e =>
              save(
                e.target.checked ? 1 : 0,
                notifyPropertySearch,
                notifyDm,
                notifyAnnounce
              )
            }
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          <input
            type="checkbox"
            className="accent-primary w-4 h-4"
            checked={notifyPropertySearch === 1}
            onChange={e =>
              save(
                notifyNewProperty,
                e.target.checked ? 1 : 0,
          ...
          />
          <div>
            <span className="text-sm font-medium">物件募集のお知らせ</span>
            <p className="text-xs text-muted-foreground">
              前日に公開された物件募集を朝10時にまとめてお知らせ
            </p>
          </div>
        </label>
        <label className="flex items-center gap-3 cursor-pointer">
  ...
  return (
    ...
    >
      <button
        className={
          v2
            ? "flex w-full items-center justify-between bg-[#edf1f5] px-5 py-4 text-left"
            : "w-full px-5 py-3 flex items-center justify-between text-left bg-muted/40"
        }
  ...
  return (
    <div
      className={
        v2
          ? "overflow-hidden border border-[#d4dde7] bg-white"
          : "bg-card border border-border rounded-lg p-4 md:p-6 space-y-4"
      }
    >
      <div
        className={
          v2
            ? "flex items-start justify-between bg-[#173f70] p-5 text-white sm:p-6"
      ...
      >
        <div className="flex items-start gap-3 md:gap-5">
          <div
            className={
              v2
                ? "grid size-14 shrink-0 place-items-center bg-white/15"
                : "w-12 h-12 md:w-16 md:h-16 bg-primary/10 rounded-full flex items-center justify-center shrink-0"
            }
          >
            <span
              className={
          ...
          <div>
            <h2
              className={
                v2
                  ? "text-[21px] font-bold text-white"
            ...
            <div className="flex items-center gap-3 mt-1">
              <span
                className={
                  v2
                    ? "bg-white/15 px-2 py-1 text-[11px] font-bold text-white"
                    : "text-xs font-medium px-2 py-0.5 rounded bg-primary/10 text-primary"
                }
              >
        ...
        {!editing && (
          <Button
            ...
            className={
              v2
                ? "h-10 rounded-none border-white/60 bg-transparent px-4 font-bold text-white hover:bg-white/10 hover:text-white"
                : "gap-1.5"
            }
            onClick={startEdit}
          >
      ...
      {editing ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                value={form.holidays}
                onChange={e =>
                  setForm(p => ({ ...p, holidays: e.target.value }))
                }
                placeholder="土日祝"
              />
            </div>
        ...
        <div
          className={
            v2
              ? "grid grid-cols-1 gap-x-8 gap-y-0 p-5 text-[14px] sm:grid-cols-2 sm:p-6"
              : "grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-sm pt-2 border-t border-border"
          }
        >
          {infoItems.map(item => (
            <div
              ...
              className={
                v2
                  ? "flex min-w-0 items-center gap-2 border-b border-[#e1e6ec] py-3 text-[#526176]"
            ...
            >
              ...
              <span
                className={item.value ? "" : "text-muted-foreground/40 text-xs"}
              >
                {item.value || "未設定"}
              </span>
            </div>
...
function InterestedUsersSection() {
  ...
  const byProperty = new Map<
    number,
    { propertyName: string; users: typeof data }
  >();
  for (const entry of data) {
    if (!byProperty.has(entry.propertyId)) {
      byProperty.set(entry.propertyId, {
