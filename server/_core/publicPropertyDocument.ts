type PublicPropertyDocumentData = {
  ...
  externalPriceVisible?: number | boolean | null;
...
export function buildPublicPropertyDocumentHtml(property: PublicPropertyDocumentData, inquiryUrl: string) {
  ...
  if (property.externalPriceVisible) {
    rows.unshift(["価格", property.priceNegotiable ? "応相談" : property.price ? `${property.price.toLocaleString("ja-JP")}円` : "-", "", ""]);
  }
