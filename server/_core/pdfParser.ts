...
function detectMediaType(fileName: string): { type: "document" | "image"; media_type: string } {
  if (IMAGE_EXTS.test(fileName)) {
    const ext = fileName.split(".").pop()?.toLowerCase() ?? "jpeg";
    const mimeMap: Record<string, string> = { jpg: "image/jpeg", jpeg: "image/jpeg", png: "image/png", gif: "image/gif", webp: "image/webp" };
    return { type: "image", media_type: mimeMap[ext] || "image/jpeg" };
  }
  return { type: "document", media_type: "application/pdf" };
}

export async function parsePropertyFromPdfs(filesBase64: string[], fileNames?: string[]): Promise<{
  ...
  try {
    ...
    for (let i = 0; i < filesBase64.length; i++) {
      ...
      const name = fileNames?.[i] ?? "file.pdf";
      const { type, media_type } = detectMediaType(name);
      content.push({
        type,
