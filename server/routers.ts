...
画像が横向き、縦向き、上下逆の場合は、90度・180度・270度回転した状態も想定して全方向から文字を確認してください。
氏名は最も大きく記載された個人名を優先し、会社名、部署名、役職名を氏名として扱わないでください。姓または名だけで断定せず、可能な限りフルネームを読み取ってください。
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (ok) {
          ...
          await db.sendDirectMessage(
            ctx.user.id,
            input.partnerId,
            `✉️ ${sentItemsLabel}のメールを送信しました${documentNames.length ? `\n${documentNames.map(name => `・${name}`).join("\n")}` : ""}\n受信まで5分程度かかる場合があります。届かない場合は、受信トレイを更新し、迷惑メールフォルダーもご確認ください。`,
            input.propertyId
          );
          db.logActivity(
            ctx.user.id,
