...
try {
  ...
  const listedDraft = sellerPropertyList.find((item: any) => item.id === draftCreated.data.id);
  assert(listedDraft?.published === 0, "property registrant sees own draft with unpublished state in self-property list data");
